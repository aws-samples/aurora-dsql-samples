...
initial connection to DSQL from the Lambda will be the slowest, but future usages will be considerably faster.
...
async function dsql_sample(clusterEndpoint, region) {
  let pool;
  try {
    pool = await getPool(clusterEndpoint, region)
...
async function getPool(endpoint, region) {
...
async function createTable(pool) {
  let client = await getClientFromPool(pool);
  // Create a new table
  let start = Date.now();
