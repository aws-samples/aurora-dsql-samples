...
async function sequelizeExample(sequelize: Sequelize) {
  ...
  const carlosResult = await Vet.findOne({
    ...
    order: [
      [{ model: Specialty, as: 'Specialties' }, 'name', 'ASC']
    ]
  });
  ...
  if (akuaResult?.Specialties?.[0]) {
    const specialty = akuaResult.Specialties[0].id;
    console.log(`Exotic ID: ${specialty}`);
    if (specialty != "Exotic") throw new Error(`Incorrect query result, expected: "Exotic", actual: ${specialty}`);
  }

  if (carlosResult?.Specialties?.[0]) {
    const specialty = carlosResult.Specialties[0].id;
    console.log(`Cats ID: ${specialty}`);
    if (specialty != "Cats") throw new Error(`Incorrect query result, expected: "Cats", actual: ${specialty}`);
  }

  if (carlosResult?.Specialties?.[1]) {
    const specialty = carlosResult.Specialties[1].id;
    console.log(`Dogs ID: ${specialty}`);
    if (specialty != "Dogs") throw new Error(`Incorrect query result, expected: "Dogs", actual: ${specialty}`);
  }
}
...
async function retryExample(sequelize: Sequelize) {
...
export async function runExamples() {
  var sequelize = await getSequelizeConnection();
  await sequelizeExample(sequelize);
  await retryExample(sequelize);
