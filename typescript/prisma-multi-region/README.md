...
### 3. Generate incremental migrations

For schema changes, diff the old schema against the new one and transform:
...
The `aurora-dsql-prisma` tools wrap each DDL in `BEGIN`/`COMMIT` blocks and convert indexes to `CREATE INDEX ASYNC`.
...
### 5. Generate Prisma client and start
...
These tests cover basic connectivity, CRUD operations, relations, foreign key enforcement, and UUID generation.
