...
Migrations are applied via `psql` (not `prisma migrate deploy`) because DSQL requires one DDL statement per transaction. The migration file generated by `aurora-dsql-prisma` wraps each statement in `BEGIN`/`COMMIT` blocks, which `psql` executes individually.

```bash
# Requires PGHOST, PGUSER, PGPASSWORD, PGSSLMODE env vars
